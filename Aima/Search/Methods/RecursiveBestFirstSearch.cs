    ...
    public class RecursiveBestFirstSearch<TState> : HeuristicSearch<TState>
    {
        ...
        public override ISolution<TState> Search(IProblem<TState> problem)
        {
            return new Solution<TState>(
                RBFS(problem,
                    new HeuristicTreeNode<TState>(problem.InitialState, Expander.ComputeHeuristic(problem.InitialState)),
                    double.PositiveInfinity).Item1);
        }

        private Tuple<HeuristicTreeNode<TState>, double> RBFS(IProblem<TState> problem, HeuristicTreeNode<TState> node,
            double fLimit)
        {
            // Notify new state
        ...
        public class RBFSExpander<T> : HeuristicNodeExpander<T>
        {
            ...
            public override IEnumerable<HeuristicTreeNode<T>> Expand(HeuristicTreeNode<T> node, IProblem<T> problem)
            {
                ...
                foreach (var successor in successors)
                {
                    ...
                    var f = Math.Max(node.PathCost + ComputeHeuristic(state), node.F);
                        // like in book, not optimal solution (but close)
